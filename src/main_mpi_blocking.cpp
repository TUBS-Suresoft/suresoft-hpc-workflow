...
int main(int argc, char **argv)
{
    ...
    while (!done)
    {
        ...
        if (myRankX % 2 == 0)
        {
            if (!isRightBoundaryCell)
            {
                MPI_Ssend(rightSendBuffer, innerGrid.ny(), MPI_DOUBLE, processTopology.pos(myRankX + 1, myRankY), 0, MPI_COMM_WORLD);
                MPI_Recv(rightReceiveBuffer, innerGrid.ny(), MPI_DOUBLE, processTopology.pos(myRankX + 1, myRankY), 0, MPI_COMM_WORLD, &status);
            }
        }
        ...
        else
        {
            if (!isLeftBoundaryCell)
            {
                MPI_Recv(leftReceiveBuffer, innerGrid.ny(), MPI_DOUBLE, processTopology.pos(myRankX - 1, myRankY), 0, MPI_COMM_WORLD, &status);
                MPI_Ssend(leftSendBuffer, innerGrid.ny(), MPI_DOUBLE, processTopology.pos(myRankX - 1, myRankY), 0, MPI_COMM_WORLD);
            }
        }
        ...
        if (myRankY % 2 == 0)
        {
            if (!isBottomBoundaryCell)
            {
                MPI_Ssend(bottomSendBuffer, innerGrid.nx(), MPI_DOUBLE, processTopology.pos(myRankX, myRankY + 1), 0, MPI_COMM_WORLD);
                MPI_Recv(bottomReceiveBuffer, innerGrid.nx(), MPI_DOUBLE, processTopology.pos(myRankX, myRankY + 1), 0, MPI_COMM_WORLD, &status);
            }
        }
        ...
        else
        {
            if (!isTopBoundaryCell)
            {
                MPI_Recv(topReceiveBuffer, innerGrid.nx(), MPI_DOUBLE, processTopology.pos(myRankX, myRankY - 1), 0, MPI_COMM_WORLD, &status);
                MPI_Ssend(topSendBuffer, innerGrid.nx(), MPI_DOUBLE, processTopology.pos(myRankX, myRankY - 1), 0, MPI_COMM_WORLD);
            }
        }
